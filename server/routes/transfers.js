		...
		async (req, res) => {
			...
			if (!validStatuses.includes(status)) {
				return res
					.status(400)
					.send(
						'Invalid status. Must be one of: requested, accepted, delivered',
					);
			}
			try {
		...
		async (req, res) => {
			...
			try {
				...
				if (amountSent != null) {
					...
					console.log(
						'Amount sent updated to: ',
						amountSent,
					);
				}
				if (amountSentType != null) {
					transfer.amountSentType =
						amountSentType.toString();
					console.log(
						'Amount sent type updated to: ',
						amountSentType,
					);
				}
				if (sendingUnit != null) {
